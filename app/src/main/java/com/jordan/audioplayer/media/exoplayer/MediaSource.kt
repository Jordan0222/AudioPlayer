...
class MediaSource @Inject constructor(
    ...
    suspend fun load() {
        state = AudioSourceState.STATE_INITIALING
        ...
        state = AudioSourceState.STATE_INITIALED
    }
    ...
    private val isReady: Boolean
        get() = state == AudioSourceState.STATE_INITIALED

    // 確認是否加載完畢
    fun whenReady(listener: OnReadyListener): Boolean {
        return if (state == AudioSourceState.STATE_CREATED || state == AudioSourceState.STATE_INITIALING) {
